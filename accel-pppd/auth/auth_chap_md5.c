...
static void auth_result(struct chap_auth_data *ad, int res)
{
	...
	if (res == PWDB_DENIED || res == PWDB_DENIED_SECOND) {
		...
		if (ad->started) {
			...
			ppp_auth_failed(ad->ppp, name, (res == PWDB_DENIED_SECOND));
	} else {
		if (!ad->started) {
...
static void chap_recv_response(struct chap_auth_data *ad, struct chap_hdr *hdr)
{
	...
	if (r == PWDB_NO_IMPL) {
		...
		_free(passwd);
	} else if (r == PWDB_DENIED || r == PWDB_DENIED_SECOND) {
		chap_send_failure(ad);
		if (ad->started) {
			ap_session_terminate(&ad->ppp->ses, TERM_USER_ERROR, 0);
			_free(name);
		} else
			ppp_auth_failed(ad->ppp, name, (r == PWDB_DENIED_SECOND));
	} else {
		if (!ad->started) {
			if (ppp_auth_succeeded(ad->ppp, name)) {
				chap_send_failure(ad);
